...
ResyncLabel = uicontrol(h, 'Style', 'text', 'Units','Normalized', 'Position',...
    [.15 0.50 0.4 0.03],'String', 'ReSync Time Window(s):');
...
ResyncButton = uicontrol(h, 'Style', 'pushbutton', 'Units','Normalized', 'Position',...
    ...
    'callback','data = guidata(gcf);data.glb = 0;guidata(gcf,data);EEG = RS_resyncERP(EEG,guidata(gcf));');

ResyncGlbButton = uicontrol(h, 'Style', 'pushbutton', 'Units','Normalized', 'Position',...
    [0.2 .25 0.6 0.05],'String', {'ReSync Globally (overwrite EEG)'},...
    'callback','data = guidata(gcf);data.glb = 1;guidata(gcf,data);EEG = RS_resyncERP(EEG,guidata(gcf));');
